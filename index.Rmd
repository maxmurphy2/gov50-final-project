...
A 1-unit increase in X (1 degrees C) is associated with an average increase in 676.9 annual fatalities.

(Note: intercept doesn't really mean much as the avg temp is so high, never 0)
...
A 1-unit increase in X (1mm of average annual precipitation) is associated with an average decrease in 21.13 annual fatalities. 
...
stargazer(fit_precip_temp, type = "text")
```
...
avg_monthly_temp_fatalities_scatter <- ggplot(avg_monthly_temp_precip_fatalities,
  ...
  labs(
    ...
    title = "Avg Monthly Temp & Avg Monthly Fatalities") +
  geom_smooth(method = "lm", se = F, color = "indianred1", size = 1.5)
...
stargazer(fit_3, type = "text")
```
A 1-unit increase in X (1 degrees C in average monthly temperature) is associated with an average decrease in 3.71 average monthly fatalities.

All of this makes me think that clearly it's not down to seasonality - i.e. it's due to ANNUAL increases in temperature, not changes within the year. 
...
avg_monthly_precip_fatalities_scatter <- ggplot(avg_monthly_temp_precip_fatalities,
  ...
  labs(
    ...
    y = "Average monthly fatalities",
    title = "Avg Monthly Precip & Avg Monthly Fatalities") +
  geom_smooth(method = "lm", se = F, color = "indianred1", size = 1.5)
...
stargazer(fit_4, type = "text")
```
A 1-unit increase in X (1mm of average monthly precipitation) is associated with an average increase in 0.43 monthly fatalities. 
